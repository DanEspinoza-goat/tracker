...
function render() { const content = screen === 'queue' ? renderQueue(state, view) : screen === 'myRequests' ? renderMyRequests(state) : screen === 'currentCapacity' ? renderCapacity(state) : screen === 'archive' ? renderArchive(state) : screen === 'qc' ? renderQcWorklist(state) : renderWorkspace(state.myRequests.find(r=>r.id===activeRequestId)); app.innerHTML = renderShell(content, state, screen); bind(); initialiseClocks(); }
function bind() {
...
function bindQueue() {
  ...
  app.querySelectorAll('[data-sort]').forEach(el => el.onclick=()=>{ const key=el.dataset.sort; if (!['expand','attachments','mail','assign','project','urgent'].includes(key)) { view.sort=toggleSort(view.sort,key); render(); } });
  ...
  app.querySelectorAll('[data-assign]').forEach(el=>el.onclick=()=>confirmAssignment(Number(el.dataset.assign)));
  app.querySelector('.new-request').onclick=()=>alert('New request entry will be delivered with the Request Workspace screen.');
}
