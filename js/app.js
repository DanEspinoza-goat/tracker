...
import { showNewRequestDialog, showRequestDialog, showEditableQueueRequest, showEmailDialog } from './request-dialog.js';
...
function bind() {
  ...
  app.querySelectorAll('[data-submit-new-request]').forEach(el => el.onclick=()=>showNewRequestDialog(createNewRequest));
  ...
  app.querySelectorAll('[data-day-action]').forEach(button => button.onclick = () => { state.dayStatus=button.dataset.dayAction; state.dayStatusAt=new Date().toISOString(); saveState(state); render(); });
  ...
  app.querySelectorAll('[data-compose-email]').forEach(el=>el.onclick=event=>{ event.stopPropagation(); const request=findRequest(el.dataset.composeEmail); if (request) showEmailDialog(request,()=>{ saveState(state); render(); }); });
...
function bindQueue() {
  ...
  app.querySelector('.new-request')?.addEventListener('click',()=>showNewRequestDialog(createNewRequest));
